...
for i_fold, (train_index, test_index) in enumerate(kf_out.split(X, Y)):       # noqa
    ...
    results_training = compute_results(i_fold, "Admission Train _Imp", pred_train, Y_train_whole, results_training)                 # noqa
    ...
    results_direct = compute_results(i_fold, "Admission_imp", pred_test, Y_test, results_direct)                 # noqa
...
results_direct_df.to_csv(save_dir+ "Admission_compleat_new_hyper.csv")                    # noqa
results_training_df.to_csv(save_dir+ "Admission_compleat_training_new_hyper.csv")                  # noqa


predictions_full = pd.DataFrame(predictions_full)
predictions_full = predictions_full.T
predictions_full.to_csv(save_dir+ "predictions_Admission_complete_new_hyper.csv")   # noqa


results_LG.to_csv(save_dir+ "LG_Admission_compleat_new_hyper.csv")              # noqa

results_LG = pd.DataFrame(predictions_LG)
results_LG = results_LG.T
results_LG.to_csv(save_dir+ "predictions_LG_Admission_complete_new_hyper.csv")   # noqa

y_true_loop = pd.DataFrame(y_true_loop)
y_true_loop = y_true_loop.T
y_true_loop.to_csv(save_dir+ "y_true_Admission_complete_imp_new_hyper.csv")   # noqa


results_estimators.to_csv(save_dir+ "Best_Admission_model_parameters_complete_new_hyper.csv")   # noqa
