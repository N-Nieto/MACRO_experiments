...
for i_fold, (train_index, test_index) in enumerate(kf_out.split(X, Y)):       # noqa
    ...
    X_train_whole = round_columns_with_different_digits(imp_mean.fit_transform(X_train_whole), rounded_digits)      # noqa
    # impute test data, round for matching with the original distribution
    X_test = round_columns_with_different_digits(imp_mean.transform(X_test), rounded_digits)                        # noqa
...
results_direct_df.to_csv(save_dir+ "Full_imp_big_experiment_direct_remove.csv")                    # noqa
results_training_df.to_csv(save_dir+ "Full_imp_big_experiment_training.csv")                  # noqa

results_estimators.to_csv(save_dir+ "Best_Full_imp_model_parameters.csv")   # noqa

predictions_full = pd.DataFrame(predictions_full)
predictions_full = predictions_full.T
predictions_full.to_csv(save_dir+ "predictions_Full_imp.csv")   # noqa

y_true_loop = pd.DataFrame(y_true_loop)
y_true_loop = y_true_loop.T
y_true_loop.to_csv(save_dir+ "y_true_Full_imp.csv")   # noqa
