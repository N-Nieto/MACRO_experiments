...
def compute_CardSchock_score(data):

    """
    Age >75 years 1                     PATIENT_DATA	had_dem_age_yr	Age [year]
    Confusion at presentation 1         PATIENT_DATA	had_sy_ams_yn	Altered mental status
    Previous MI or CABG 1               PATIENT_DATA	p_mh_mi_yn	Previous myocardial infarction

    ACS aetiology 1
    LVEF <40% 1                         PATIENT_DATA	hpr_echo_lvef_pct	Acute LVEF [%]

    Blood lactate <2 mmol/L 0           "admission_lactate",        # 26 Admission Lactate: Pre-PCI or Post-PCI if Pre is missing        check units                   # noqa
    2–4 mmol/L 1
    >4 mmol/L 2

    eGFRCKD-EPI >60 mL/min/1.73 m2 0    CLIP	CKDEPI	GFR CKD-EPI creatinine equation	GFR CKD-EPI creatinine equation V1 in ml/min/1.73m²
    30–60 mL/min/1.73 m2 1
    <30 mL/min/1.73 m2 2

    Age = data["had_dem_age_yr"] 
    alterated_mental_status = data["had_sy_ams_yn"]
    CABG = data["had_sy_ams_yn"]
    ACS = 0
    LVEF = data["hpr_echo_lvef_pct"]
    lactate = data["admission_lactate"]
    eGRF = data["ckdepi"]

    Calculate CardShock risk score based on the provided criteria and save it in the dataframe.

    Args:
    dataframe (DataFrame): Pandas DataFrame representing the dataset where each column corresponds to each feature
                           and each row corresponds to a patient.

    Returns:
    None: The CardShock risk scores are calculated and saved in the 'CardShock_Score' column of the dataframe.
    """
    missing_values = data[['had_dem_age_yr', 'had_sy_ams_yn', 'p_mh_mi_yn',
                           'hpr_echo_lvef_pct',
    ...
    # Age > 75 years
    scores[~missing_values] = (data['had_dem_age_yr'] > 75).astype(int)

    # Confusion at presentation
    scores[~missing_values] += data['had_sy_ams_yn']

    # Previous MI or CABG
    scores[~missing_values] += data['p_mh_mi_yn']

    # ACS aetiology / All patients are AMI, that is more severe thatn ACS
    scores += 1

    # LV EF < 40%
    scores[~missing_values] += (data['hpr_echo_lvef_pct'] < 40).astype(int)

    # Blood lactate
    scores[~missing_values] += (data['admission_lactate'] > 4) + (data['admission_lactate'] > 2)

    # eGFR CKD-EPI
    scores[~missing_values] += (data['ckdepi'] < 30) + ((data['ckdepi'] >= 30) & (data['ckdepi'] <= 60))

    # Assign the calculated scores to the 'CardShock_Score' column
    data['CardShock_Score'] = scores
