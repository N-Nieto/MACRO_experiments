...
X_eicu, Y_test_eicu = load_eICU(features="Full", exclude_smokers=True, X_CULPRIT=X)
...
full_model = get_inner_loop_optuna(X, Y, kf_inner, params_optuna)
...
results_full = compute_results(
    1, "Full Test (eICU)", pred_test, Y_test_eicu, results_full
)

pred_train = full_model["model"].predict_proba(X)[:, 1]

# Compute test metrics
results_full = compute_results(1, "Full Train (CULPRIT)", pred_train, Y, results_full)
